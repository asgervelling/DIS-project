  ...
  switch (period) {
    case "24h":
      return {
        ...
        previous:
          "tr.date >= now() - interval '2 day' AND tr.date < now() - interval '1 day'",
      };
    case "7d":
      return {
        current: "tr.date >= now() - interval '7 days'",
        previous:
          "tr.date >= now() - interval '14 days' AND tr.date < now() - interval '7 days'",
      };
    case "30d":
      return {
        current: "tr.date >= now() - interval '30 days'",
        previous:
          "tr.date >= now() - interval '60 days' AND tr.date < now() - interval '30 days'",
      };
    case "ytd":
      return {
        current: "tr.date >= date_trunc('year', now())",
        previous:
          "tr.date >= date_trunc('year', now() - interval '1 year') AND tr.date < date_trunc('year', now())",
      };
    case "1y":
      return {
        current: "tr.date >= now() - interval '1 year'",
        previous:
          "tr.date >= now() - interval '2 years' AND tr.date < now() - interval '1 year'",
      };
    case "all":
      return {
        current: "true",
